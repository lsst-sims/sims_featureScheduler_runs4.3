...
def optimize_ddf_times(
    ...
    return night, night_season, night_mask
...
def generate_ddf_scheduled_obs(
    ...
    for ddf_name in ddf_names:
        ...
        night, night_season, night_mask = optimize_ddf_times(ddf_name, ddfs[ddf_name][0], ddf_grid)

        potential_nights = np.unique(np.where(night_mask == 1)[0])

        night_label = np.arange(potential_nights.size)

        which_sequence = night_label % (np.max(list(master_sequences.keys())) + 1)

        for snight, seq_indx in zip(potential_nights, which_sequence):
            sequence = master_sequences[seq_indx]
            for bandname in sequence:
                nexp = nexps[bandname]
                potential_mjds = np.where(night == snight)[0]
                m5s = ddf_grid[ddf_name+'_m5_g'][potential_mjds]
                best_m5_indx = np.min(np.where(m5s == np.nanmax(m5s))[0])
                mjd = ddf_grid["mjd"][potential_mjds[best_m5_indx]]

                nvis = sequence[bandname]
