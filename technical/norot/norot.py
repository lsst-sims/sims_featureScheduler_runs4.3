...
class ZeroRotTelDetailer(detailers.BaseDetailer):
    """
    ...
    def __call__(self, observation_array, conditions):
        ...
        observation_array["rotSkyPos"] = np.nan
        observation_array["rotTelPos_backup"] = 0.
        observation_array["rotSkyPos_desired"] = np.nan #np.radians(self.rc.rottelpos2rotskypos(0.0, obs_pa))
        observation_array["rotTelPos"] = 0.
...
def blob_for_long(
    ...
    for bandname, bandname2 in zip(band1s, band2s):
        ...
        detailer_list.append(ZeroRotTelDetailer())
        surveys.append(
            BlobSurvey(
                basis_functions,
                weights,
