...
def ddf_slopes(
    ...
    print(ddf_name, 'season lengths', season_length)
    ...
    for i, season in enumerate(season_list):
        ...
        to_sum = raw_obs[this_season] + 0
        nz = np.where(to_sum > 0)[0]
        to_sum[nz[0:acourd_days]] *= acour_scale
        to_sum[nz[-acourd_days:]] *= acour_scale

        cumulative = np.cumsum(to_sum)
        if cumulative.max() > 0:
...
def optimize_ddf_times(
    ...
    cumulative_desired = ddf_slopes(
        ...
        acourd_days=acourd_days,
        acour_scale=acourd_scale,
    )
    ...
    for ddf_name in ddf_kwargs:
        ...
        for mjd in mjds:
            for bandname, nvis, nexp in zip(bands, nvis_master, nsnaps):
                if bandname in skip_ahead_bands:
                    mjd_skip = 1.
                else:
                    mjd_skip = 0
                if "EDFS" in ddf_name:
                    ...
                    obs["mjd"] = mjd + mjd_skip
                    obs["flush_by_mjd"] = mjd + flush_length + mjd_skip
                    obs["exptime"] = expt
                    obs["band"] = bandname
                    obs["nexp"] = nexp
