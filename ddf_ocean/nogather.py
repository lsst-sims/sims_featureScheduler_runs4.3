...
class BandSortDetailer(detailers.BaseDetailer):
    ...
    def __init__(self, desired_band_order="ugrizy", loaded_fist=True, **kwargs):
        self.desired_band_order = desired_band_order
        self.loaded_fist = loaded_fist

    def __call__(self, observation_array, conditions):

        order_to_set = copy.copy(self.desired_band_order)
        if self.loaded_fist:
            order_to_set.replace(conditions.current_band, "")
...
def gen_scheduler(args):
    ...
    ddf_config_file = args.ddf_config_file
